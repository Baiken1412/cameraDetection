...
class Database:
            ...
            if conn:
                try:
                    with conn.cursor() as cursor:
                        sql = "SELECT id, fjmc, gnslx, ip, dk, tdh, xh, zh, mm, rtspssl FROM sacw_roomip WHERE rtspssl LIKE 'usb:%' LIMIT 1"
                        cursor.execute(sql)
    ...
    def get_all_mjxx(self) -> List[Dict[str, Any]]:
        """查询所有有效的民警信息"""
        conn = self._get_conn()
        if not conn:
            return []
        try:
            with conn.cursor() as cursor:
                sql = """
                    SELECT id, mjjh, mjxm, mjxb, mj_dept, isdel, sfzh, sjh
                    FROM sacw_mjxx
                    WHERE isdel = '0' OR isdel IS NULL
                """
                cursor.execute(sql)
                rows = cursor.fetchall()
                logger.info(f"查询到 {len(rows)} 条民警信息 (sacw_mjxx)")
                return rows
        except Exception as e:
            logger.error(f"查询民警信息失败: {e}")
            return []
        finally:
            conn.close()

    def is_valid_mj_name(self, name: str) -> bool:
        """验证民警姓名"""
        if not name:
            return False
        conn = self._get_conn()
    ...
    def save_detection_record(
        ...
        sql = """
            INSERT INTO sacw_hdgj (pssj, jssj, pstp, qyid, qymc, sxtmx, rysl, jscs)
    ...
    def get_last_record_time(self, qyid: int) -> Optional[datetime]:
        ...
        try:
            with conn.cursor() as cursor:
                sql = "SELECT MAX(pssj) as last_time FROM sacw_hdgj WHERE qyid = %s"
    ...
    def update_record_end_time(self, record_id: int, jssj: datetime) -> bool:
        ...
        try:
            with conn.cursor() as cursor:
                sql = "UPDATE sacw_hdgj SET jssj = %s WHERE id = %s"
    ...
    def update_record_people_count_if_higher(self, record_id: int, new_count: int) -> bool:
        ...
        try:
            with conn.cursor() as cursor:
                cursor.execute("SELECT rysl FROM sacw_hdgj WHERE id = %s", (record_id,))
                row = cursor.fetchone()
                current = row.get('rysl') if row else None

                if current is None or current < new_count:
                    cursor.execute(
                        "UPDATE sacw_hdgj SET rysl = %s WHERE id = %s",
                        (new_count, record_id),
                    )
    ...
    def update_record_image(self, record_id: int, image_url: str) -> bool:
        ...
        try:
            with conn.cursor() as cursor:
                sql = "UPDATE sacw_hdgj SET pstp = %s WHERE id = %s"
    ...
    def update_trajectory_detection(self, record_id: int, jscs: int, jssj: datetime, rysl: int = None) -> bool:
        ...
        try:
            with conn.cursor() as cursor:
                if rysl is not None:
                    sql = "UPDATE sacw_hdgj SET jscs = %s, jssj = %s, rysl = GREATEST(COALESCE(rysl, 0), %s) WHERE id = %s"
                    cursor.execute(sql, (jscs, jssj, rysl, record_id))
                    logger.info(f"更新轨迹检测次数成功 - 记录ID: {record_id}, 检测次数: {jscs}, 结束时间: {jssj}, 人员数量: {rysl}")
                else:
                    sql = "UPDATE sacw_hdgj SET jscs = %s, jssj = %s WHERE id = %s"
    ...
    def save_trajectory_screenshot(
        ...
        sql = """
            INSERT INTO sacw_hdgj_screenshot (hdgj_id, screenshot_url, screenshot_time, screenshot_order)
            VALUES (%s, %s, %s, %s)
        """

        try:
            with conn.cursor() as cursor:
                cursor.execute(sql, (hdgj_id, screenshot_url, screenshot_time, screenshot_order))
                screenshot_id = cursor.lastrowid
                logger.info(f"保存轨迹截图成功 - 轨迹ID: {hdgj_id}, 截图ID: {screenshot_id}")
                return screenshot_id
        except Exception as e:
            logger.error(f"保存轨迹截图失败 - 轨迹ID: {hdgj_id}: {e}")
            return None
        finally:
            conn.close()
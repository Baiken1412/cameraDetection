"""
ReID配置适配器
将 caseapp 的配置映射到 ReID 系统需要的 Config 类格式
"""
import os
import sys
...
class ReIDConfigAdapter:
    ...
    def INPUT_IMAGE_DIR(self):
        ...
        rtsp_config = caseapp_config.RTSP_MONITOR_CONFIG
    ...
    def CROPPED_DIR(self):
        """裁剪后的人员图片目录"""
        reid_config = caseapp_config.REID_CONFIG
    ...
    def REID_MODEL_NAME(self):
        return caseapp_config.REID_CONFIG.get('reid_model_name', 'osnet_x1_0')
    
    @property
    def REID_PRETRAINED(self):
        return caseapp_config.REID_CONFIG.get('reid_pretrained', True)
    
    # ========== 检测参数 ==========
    @property
    def DETECTION_CONF_THRESHOLD(self):
        return caseapp_config.REID_CONFIG.get('detection_conf_threshold', 0.5)
    
    @property
    def DETECTION_IOU_THRESHOLD(self):
        return caseapp_config.REID_CONFIG.get('detection_iou_threshold', 0.5)
    ...
    def SIMILARITY_THRESHOLD(self):
        return caseapp_config.REID_CONFIG.get('similarity_threshold', 0.7)
    
    @SIMILARITY_THRESHOLD.setter
    def SIMILARITY_THRESHOLD(self, value):
        """允许动态修改相似度阈值"""
        caseapp_config.REID_CONFIG['similarity_threshold'] = value
    
    @property
    def SIMILARITY_METRIC(self):
        return caseapp_config.REID_CONFIG.get('similarity_metric', 'cosine')
    
    # ========== 设备配置 ==========
    @property
    def DEVICE(self):
        device = caseapp_config.REID_CONFIG.get('device', 'cuda')
    ...
    def USE_DATABASE(self):
        return True  # caseapp 始终使用数据库
    
    @property
    def DB_HOST(self):
        return caseapp_config.REID_DATABASE_CONFIG['host']
    
    @property
    def DB_PORT(self):
        return caseapp_config.REID_DATABASE_CONFIG['port']
    
    @property
    def DB_USER(self):
        return caseapp_config.REID_DATABASE_CONFIG['user']
    
    @property
    def DB_PASSWORD(self):
        return caseapp_config.REID_DATABASE_CONFIG['password']
    
    @property
    def DB_NAME(self):
        return caseapp_config.REID_DATABASE_CONFIG['database']
    
    @property
    def DB_CHARSET(self):
        return caseapp_config.REID_DATABASE_CONFIG['charset']
    
    # ========== Flask配置 ==========
    @property
    def FLASK_HOST(self):
        return caseapp_config.REID_CONFIG.get('flask_host', '0.0.0.0')
    
    @property
    def FLASK_PORT(self):
        return caseapp_config.REID_CONFIG.get('flask_port', 5000)
    
    @property
    def FLASK_DEBUG(self):
        return caseapp_config.REID_CONFIG.get('flask_debug', True)
    
    @property
    def SECRET_KEY(self):
        return caseapp_config.REID_CONFIG.get('secret_key', 'caseapp-reid-secret-key-2024')

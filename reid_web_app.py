"""
整合的 ReID Web 应用
包含所有 ReID 训练和管理页面，使用 bjjcspbj 的配置
"""
import logging
import sys
import os
...
# 注意：必须在添加 ReID 系统路径之前导入 bjjcspbj 的 database 模块
# 否则 Python 会从 ReID 系统的 database 模块导入
import database as bjjcspbj_database
from database import Database  # 从 bjjcspbj 的 database 模块导入
...
# ReID系统已整合到bjjcspbj项目中，直接使用本地模块
...
try:
    ...
    try:
        ...
        _bjjcspbj_database_backup = sys.modules.get('database')
        ...
        try:
            ...
            if _bjjcspbj_database_backup:
                sys.modules['database'] = _bjjcspbj_database_backup
    REID_AVAILABLE = True
            ...
            try:
                for record in records:
                    try:
                        ...
                        # 从URL路径转换为本地文件路径
                        # 例如: https://localhost:8090/profile/sacwspbj/20251212/22_xxx.jpg
                        # 转换为: D:/ruoyi/uploadPath/sacwspbj/20251212/22_xxx.jpg

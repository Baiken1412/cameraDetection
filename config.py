...
RTSP_MONITOR_CONFIG = {
    ...
    'image_save_path': 'D:/ruoyi/uploadPath/sacwspbj',  # 例如: 'D:/ruoyi/uploadPath/sacwspbj'

    # 图片访问URL前缀（用于存储到数据库的URL路径）
    # 本地开发环境：使用localhost + Java服务端口8090（HTTPS）
    'image_url_prefix': 'https://localhost:8090/profile/sacwspbj/',
    ...
    'min_change_area': 50,  # 从100降低到50，减少过滤

    # 摄像头配置获取接口（代替直接查询 sacw_roomip 表）
    ...
    'camera_api_url': 'https://localhost:8090/spxz/hdgj/rtspStream',
...
REID_CONFIG = {
    ...
    # 注意：ReID系统已整合到bjjcspbj项目中，不再需要外部路径
    ...
    'secret_key': 'bjjcspbj-reid-secret-key-2024',  # Flask密钥
}

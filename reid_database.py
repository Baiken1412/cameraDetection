...
# ReID系统已整合到bjjcspbj项目中
# 优先使用本地模块（reid_database_modu），如果不存在则尝试外部路径（向后兼容）
bjjcspbj_dir = Path(__file__).parent
reid_database_module_dir = bjjcspbj_dir / 'reid_database_modu'

# 如果本地模块存在，不需要添加外部路径
if not reid_database_module_dir.exists():
    # 向后兼容：如果本地模块不存在，尝试外部路径
    reid_system_path = bjjcspbj_dir / config.REID_CONFIG.get('reid_system_path', '../person-re-identification-system-main')
    ...
    if reid_system_path.exists():
        ...
        logger.info(f"请确保ReID系统已整合到bjjcspbj项目中（检查 {reid_database_module_dir}）")
else:
...
def _check_reid_modules_exist():
    ...
    try:
        ...
        if not reid_system_path.exists():
            ...
            logger.error(f"请确保ReID系统已整合到bjjcspbj项目中（检查 {reid_database_module_dir}）")
            return False
...
def _import_reid_modules():
    ...
    try:
        ...
        class MockConfig:
            ...
            RESULTS_DIR = bjjcspbj_dir / 'data' / 'results'
        
        # 临时保存 bjjcspbj 的 database 模块（如果存在）
        _bjjcspbj_database_backup = sys.modules.get('database')
        # 临时移除 bjjcspbj 的 database 模块，让 Python 导入 ReID 系统的
        ...
        try:
            ...
            if reid_database_module_dir.exists():
                ...
                try:
                    ...
                    # 恢复原来的 database 模块（但不要覆盖 _bjjcspbj_database_backup，它会在外层 finally 中恢复）

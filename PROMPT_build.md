...
- 必须优先复用 `core/` 和 `utils/` 中的标准库与既有模式。
...
- 优先运行与当前任务直接相关的测试：
```bash
python -m pytest tests/ -k "相关测试名" -v
```
...
## 状态更新规则（State Update Rules）

提交成功后，更新 IMPLEMENTATION_PLAN.md：

- 将已完成的任务标记为 ✅ 已完成。
- 如发现新的问题或缺口，追加到未决问题部分。
...
2. 所有测试已通过。
3. 更改已提交。
4. IMPLEMENTATION_PLAN.md 已更新。

如遇到以下情况，应立即停止并切换至 `PROMPT_plan.md`：
